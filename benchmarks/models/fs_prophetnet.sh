...
grep -E "fairseq_v0.9.0\+fastseq_v.* prophetnet_large_160G_gigaword_model cnn_dm_bert.1k/len-1024.bin valid 32 " perf | awk '{s+=$13}END{print s/NR}' | bash range.sh 5.5 6.0
grep -E "fairseq_v0.9.0\+fastseq_v.* prophetnet_large_160G_gigaword_model cnn_dm_bert.1k/len-1024.bin valid 64 " perf | awk '{s+=$13}END{print s/NR}' | bash range.sh 7.1 7.6
grep -E "fairseq_v0.9.0\+fastseq_v.* prophetnet_large_160G_gigaword_model cnn_dm_bert.1k/len-1024.bin valid 128 " perf | awk '{s+=$13}END{print s/NR}' | bash range.sh 8 8.5
