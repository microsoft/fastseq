...
./benchmark.sh transformers+fastseq sshleifer/distilbart-cnn-12-6 cnn_dm/raw val 64/128/256 --task summarization  # take 2.5 hours
# Accuracy
grep "sshleifer/distilbart-cnn-12-6 cnn_dm/raw val " perf | awk '{if($9!="NA"){c+=1;s+=$9}}END{print s/c}' | bash range.sh 45 45.1
# Speed on V100 16GB 250W
...
# MODEL - t5
./benchmark.sh transformers t5-base wmt_en_ro/raw val 64/128 --task translation_en_to_ro          # each bs takes 5 minutes
./benchmark.sh transformers+fastseq t5-base wmt_en_ro/raw val 64/128 --task translation_en_to_ro  # each bs takes 5 minutes
# Accuracy
...
# Accuracy
# todo, fastseq result is different, bleu is higher 27.80 vs 27.89
