...
__global__ void banRepeatedTokens(long * __restrict__ tokens,
                float * __restrict__ lprobs, int max_predict_len,
                int vocab_size, int no_repeat_ngram_size) {
    auto row = blockIdx.x;
...
torch::Tensor ngram_repeat_block_cuda_forward(
  ...
  banRepeatedTokens<<<blocks, threads, shared_mem_size >>> (
           token_ptr, lprob_ptr, max_predict_len,
           vocab_size, no_repeat_ngram_size);
  return lprobs;
}
