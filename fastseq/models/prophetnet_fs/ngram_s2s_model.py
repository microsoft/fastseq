...
from fairseq import hub_utils, options, utils
...
from fastseq.models.prophetnet_fs.learned_positional_embedding import LearnedPositionalEmbedding
from fastseq.models.prophetnet_fs.ngram_multihead_attention import NgramMultiheadAttention, ngram_attention_bias
from fastseq.models.prophetnet_fs.hub_interface import ProphetNetHubInterface
...
class NgramTransformerProphetModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--relative_max_distance',
                            type=int,
                            metavar='N',
        ...
        parser.add_argument(
            '--load-sep',
            action='store_true',
            help='load pretrained [SEP] weight into [X_SEP]. ([SEP] used as eos'
                 ' in fine tuning)'
        )
    ...
    def build_model(cls, args, task):
        ...
        if args.share_all_embeddings:
            ...
            if args.encoder_embed_dim != args.decoder_embed_dim:
                raise ValueError('--share-all-embeddings requires '
                                 '--encoder-embed-dim to match '
                                 '--decoder-embed-dim')
        ...
        if args.load_from_pretrained_model is not None:
            states = torch.load(args.load_from_pretrained_model,
                                map_location='cpu')
            ...
            if num_langids_to_add > 0:
                new_lang_embed_to_add = torch.zeros(num_langids_to_add,
                                                    embed_dim)
            ...
            for position_name, target_position_length in [
                ('encoder.embed_positions.weight',
                 model.encoder.embed_positions.weight.size(0)),
                ('decoder.embed_positions.weight',
                 model.decoder.embed_positions.weight.size(0))]:
                if states[position_name].size(0) < target_position_length:
                    index_ = torch.arange(states[position_name].size(1))
                    expend_position_states = states[position_name].clone()
                    while states[position_name].size(
                        0) < target_position_length:
                        index_ = torch.cat((index_[1:], index_[:1]), dim=0)
                        states[position_name] = torch.cat([
                            states[position_name],
                            expend_position_states[:, index_]
                        ],
                                                          dim=0)
                if states[position_name].size(0) > target_position_length:
                    states[position_name] = states[
                        position_name][:target_position_length]
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.embed_positions = LearnedPositionalEmbedding(
            args.max_source_positions + 1 + self.padding_idx,
            embed_dim,
            self.padding_idx,
        )
...
class NgramTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.embed_tokens = embed_tokens
        # math.sqrt(embed_dim)  # todo: try with input_embed_dim
        self.embed_scale = None

        self.embed_positions = LearnedPositionalEmbedding(
            args.max_target_positions + 2 + self.padding_idx,
            embed_dim,
            self.padding_idx,
        )
    ...
    def forward(self,
        ...
        # x [B, (1+ngram)*T, C]
        x_list, extra = self.extract_features(prev_output_tokens, encoder_out,
    ...
    def _relative_positions_bucket(self,
        ...
        if bidirectional:
            ...
            result = result + torch.lt(n,
                                       torch.zeros_like(n)).int() * num_buckets
            n = torch.abs(n)
        else:
        ...
        val_if_large = torch.min(
            val_if_large,
            torch.ones_like(val_if_large) * (num_buckets - 1))
    ...
    def cal_pretrain_relative_positions(self, real_positions):
        ...
        # [B,T,T/S]
        main_stream_relative_positions = main_stream_relative_positions.repeat(
            1, real_positions.size(-1), 1)
        # [B,T,1]
        real_positions_main = real_positions.unsqueeze(-1)
        main_stream_relative_positions = (
            main_stream_relative_positions - real_positions_main)
        ...
        predicting_stream_relative_positions = torch.cat(
            (real_positions_shift_predicting_stream, real_positions),
            dim=-1).unsqueeze(1)
        # [B,T, 2*T]
        predicting_stream_relative_positions = (
            predicting_stream_relative_positions.repeat(
                1, real_positions.size(-1), 1))
        # [B,T, 1]
        real_positions_predicting_stream = real_positions.unsqueeze(-1)
        predicting_stream_relative_positions = (
            predicting_stream_relative_positions -
            real_positions_predicting_stream)
        i_buckets_main_stream = self._relative_positions_bucket(
            main_stream_relative_positions, bidirectional=False)
    ...
    def cal_finetune_relative_positions(self, real_positions):
        ...
        if (not hasattr(self, '_finetune_i_bucket_main_stream') or
            ...
            fake_positions = torch.arange(
                1, self.max_target_positions + 1).repeat(1, 1)
            (finetune_i_bucket_main_stream,
             finetune_i_bucket_predicting_stream) = (
                 self.cal_pretrain_relative_positions(fake_positions))
            self._finetune_i_bucket_main_stream = (
                finetune_i_bucket_main_stream.to(real_positions.device))
            self._finetune_i_bucket_predicting_stream = (
                finetune_i_bucket_predicting_stream.to(real_positions.device))
        finetune_i_bucket_main_stream = self._finetune_i_bucket_main_stream[
            :, :n_tokens, :n_tokens].repeat(batch_size, 1, 1)
        ...
        return (finetune_i_bucket_main_stream,
                finetune_i_bucket_predicting_stream)
    ...
    def extract_features(self,
                         ...
                         **unused):
        # embed positions
        # [bos, A, B, C, D, eos] with real positions [1,2,3,4,5,6](main stream),
        # [2,3,4,5,6,7](predicting stream) target [B,C,D] with prev [A,B,C] from
        # [A,B,C,D] as pretraining span with real positions [2,3,4], but target
        # actually [3,4,5] for fine tune with another [bos]. thus [2,3,4] used
        # for main stream shifted prev [A,B,C], [3,4,5] used for predicting
        # [B,C,D]
        if 'positions' in unused:
            # pretrain procedure
            main_stream_pos_embed = self.embed_positions._forward(
                unused['positions'])
        ...
        predicting_stream_pos_embed = self.embed_positions._forward(
            real_positions + 1)
        ...
        if incremental_state is not None:
            B = x.size(1)
            ngram_masks = [
                (ngram_input_embed[ngram - 1] +
                 predicting_stream_pos_embed).transpose(0, 1).repeat(1, B, 1)
                for ngram in range(self.ngram)
            ]
        else:
            ngram_masks = [(ngram_input_embed[ngram - 1] +
                            predicting_stream_pos_embed).transpose(0, 1)
                           for ngram in range(self.ngram)]

        self_attn_mask = self.buffered_future_mask(
            x) if incremental_state is None else None
        ngram_mask_matrix = self.buffered_future_mask_ngram(
            x) if incremental_state is None else None
    ...
    def buffered_future_mask(self, tensor):
        ...
        if (not hasattr(self, '_future_mask') or
            ...
            self._future_mask.device != tensor.device or
            self._future_mask.size(0) < dim):
    ...
    def buffered_future_mask_ngram(self, tensor):
        ...
        if (not hasattr(self, '_ngram_future_mask') or
            ...
            self._ngram_future_mask = ngram_attention_bias(
                ...
                self.ngram).type(tensor.dtype).to(tensor.device)
        ngram_future_mask = torch.cat([
            self._ngram_future_mask[:, :dim, :dim],
            self._ngram_future_mask[
                :, :dim,
                self.max_target_positions:self.max_target_positions + dim]], 2)
...
@register_model_architecture(
    'ngram_transformer_prophet', 'ngram_transformer_prophet')
...
@register_model_architecture('ngram_transformer_prophet',
                             'ngram_transformer_prophet_base')
...
@register_model_architecture('ngram_transformer_prophet',
                             'ngram_transformer_prophet_middle')
...
@register_model_architecture('ngram_transformer_prophet',
                             'ngram_transformer_prophet_large')
