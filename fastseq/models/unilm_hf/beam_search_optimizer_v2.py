...
class GenerationMixinV3(GenerationMixinV2):
    def calculate_banned_tokens_from_bad_words(
        ...
        for ngram_size in bad_words_dict["ngram_size"]:
            ngram_index = ",".join(
                [str(x) for x in tokens_list[-(ngram_size - 1):]])
            bad_indice += bad_words_dict.get(ngram_index,
                                             torch.jit.annotate(List[int], []))
        return bad_indice

    def _no_bad_words(self,
                      tokens,
                      lprobs,
                      bsz: int,
                      beam_size: int,
        ...
        for bbsz_idx in range(bsz * beam_size):
            lprobs[bbsz_idx][torch.tensor(
                banned_tokens[bbsz_idx]).long()] = torch.tensor(
                    -math.inf, dtype=torch.float).to(lprobs)
        return lprobs
    ...
    def _beam_search_step(self, step, scores, beam_scores):
        ...
        cand_scores, next_tokens = torch.topk(beam_scores,
                                              2 * num_beams,
                                              dim=1,
                                              largest=True,
                                              sorted=True)
    ...
    def _do_sample_step(self, step, scores, beam_scores, temperature, top_k,
                        top_p):
        batch_size, num_beams, vocab_size = scores.size()
        ...
        cand_scores, next_scores_indices = torch.sort(next_scores,
                                                      descending=True,
                                                      dim=1)
        # (batch_size, num_beams * 2)
    ...
    def _generate_beam_search(
        ...
        bbsz_offsets = ((torch.arange(0, batch_size) *
                         num_beams).unsqueeze(1).type_as(input_ids))
        cand_offsets = torch.arange(0, cand_size).type_as(input_ids)
        cands_to_ignore = torch.zeros(batch_size,
                                      num_beams).to(input_ids).eq(-1)
        ...
        if bad_words_ids is not None:
            single_bad_words = [
                ngram[0] for ngram in bad_words_ids if len(ngram) == 1
            ]
            bad_words_ids = [
                ngram for ngram in bad_words_ids if len(ngram) > 1
            ]
        ...
        while cur_len < max_length + 1:
            ...
            if repetition_penalty != 1.0:
                ...
                curr_scores = scores[torch.arange(prev_token_ids.size(0)),
                                     prev_token_ids.transpose(0, 1)].transpose(
                                         0, 1)
                rep_penalty = (torch.zeros_like(prev_token_ids).fill_(
                    repetition_penalty).to(curr_scores))
                rep_penalty = (curr_scores < 0).float() * rep_penalty + (
                    curr_scores > 0).float() / rep_penalty
            ...
            if do_sample:
                cand_scores, cand_indices, cand_beams = self._do_sample_step(
                    cur_len - start_len, scores, beam_scores, temperature,
                    top_k, top_p)
            else:
            ...
            eos_mask[:, :num_beams][cands_to_ignore] = torch.tensor(0).to(
                eos_mask)
            eos_bbsz_idx = torch.masked_select(cand_bbsz_idx[:, :num_beams],
                                               mask=eos_mask[:, :num_beams])
            finalized_sents: List[int] = []
            if eos_bbsz_idx.numel() > 0:
                eos_scores = torch.masked_select(cand_scores[:, :num_beams],
                                                 mask=eos_mask[:, :num_beams])
                ...
                for i in range(eos_bbsz_idx.size(0)):
                    ...
                    if len(finalized[sent]) < num_beams:
                        finalized[sent].append({
                            ...
                            input_clone[i],
                            "score":
                            score / (cur_len - start_len + 1)**length_penalty,
                        })
                for seen in sents_seen.keys():
                    sent = int(float(seen.split("_")[0]))
                    unfin_idx = int(float(seen.split("_")[1]))
                    if not finished[sent] and self.is_finished(
                            cur_len, unfin_idx, max_length, len(
                                finalized[sent]), num_beams):
                        finished[sent] = True
            ...
            eos_mask[:, :num_beams] = ~((~cands_to_ignore) &
                                        (~eos_mask[:, :num_beams]))
            ...
            new_token = torch.gather(cand_indices, dim=1,
                                     index=active_hypos).view(-1)
        ...
        decoded = (torch.zeros(
            (output_batch_size,
             max_length)).long().to(input_ids).fill_(pad_token_id))
    ...
    def is_finished(self, cur_len, unfin_idx, max_length, finalized_sent_len,
                    num_beams):
